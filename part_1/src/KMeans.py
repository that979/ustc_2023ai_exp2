...
class KMeans:
    ...
    def assign_points(self, centers, points):
        ...
        for i in range(n_samples):
            square_dist = np.sum((centers - points[i]) ** 2, axis=1)
            labels[i] = np.argmin(square_dist)
    ...
    def update_centers(self, centers, labels, points):
        ...
        for i in range(n_samples):
            centers[int(labels[i])] += points[i]
            cnt[int(labels[i])] += 1

        centers /= cnt[:, np.newaxis]
    ...
    def compress(self, img):
        ...
        labels,centers=self.fit(points)

        for label in range(self.k):
            points[labels == label] = centers[label]
